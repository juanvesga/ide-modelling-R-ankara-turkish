...
# Parameter values per day
  CFR<- 0.15 # <---------------------- FIll the gap
  
  gamma <- 1/6 # Recovery rate # <---------------------- FIll the gap
  
  beta <- 0.6  # infection rate # <---------------------- FIll the gap
  
  mu <- gamma*CFR/(1-CFR) ## # <---------------------- FIll the gap
...
# Extract peak value and time

id <- which(output$I == max(output$I)) 
peak_time<- output$time[id]
peak_time

peak_value<-output$I[id]
peak_value


final_size<- output$R[length(output$R)] + output$M[length(output$R)]
final_size
